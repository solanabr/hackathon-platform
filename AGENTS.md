...
- Use `createServiceRoleClient()` only in the wired places: role resolution (`src/lib/roles.ts`), role management (`/admin/pessoas`), cron lock (`/api/cron/lock-submissions`), public finalists section (`(public)/h/[slug]/page.tsx`).
