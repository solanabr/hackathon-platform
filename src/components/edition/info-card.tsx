...
export function EditionInfoCard({ hackathon }: { hackathon: Hackathon }) {
  ...
  const rows = [
    hackathon.registration_closes_at
      ? { label: "Inscrições até", when: hackathon.registration_closes_at, detail: d(hackathon.registration_closes_at) }
      : null,
    {
      label: "Submissão até",
      when: hackathon.submission_deadline_at,
      detail: `${d(hackathon.submission_deadline_at)}, ${TIME.format(new Date(hackathon.submission_deadline_at))}`,
    },
    hackathon.finalists_announced_at
      ? { label: "Finalistas", when: hackathon.finalists_announced_at, detail: d(hackathon.finalists_announced_at) }
      : null,
    hackathon.presential_at
      ? { label: "Pitch Day", when: hackathon.presential_at, detail: d(hackathon.presential_at) }
      : null,
  ].filter((r): r is { label: string; when: string; detail: string } => r !== null);
